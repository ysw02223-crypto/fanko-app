...
const ORDER_FIELD_LABELS: Record<OrderEditableField, string> = {
  customer_name: "Í≥†Í∞ùÎ™?,
  purchase_channel: "Í±∞ÎûòÏ≤?,
};

const ITEM_FIELD_LABELS: Record<ItemEditableField, string> = {
  product_type: "Ïπ¥ÌÖåÍ≥†Î¶¨",
  product_name: "?ÅÌíàÎ™?,
  product_option: "?µÏÖò",
  product_set_type: "?®Ìíà/?∏Ìä∏",
  quantity: "?òÎüâ",
  price_rub: "?êÎß§Í∞Ä??,
  prepayment_rub: "?†Í≤∞?ú‚ÇΩ",
  krw: "?êÌôîÎß§ÏûÖ",
  progress: "ÏßÑÌñâ",
  gift: "?†Î¨º",
  photo_sent: "?¨ÏßÑ",
...
export function OrdersLineItemsTable({ initialOrders }: { initialOrders: OrderWithNestedItems[] }) {
  ...
  useEffect(() => {
    ...
    /** ?úÎûòÍ∑??§ÌÅ¨Î°??úÏûëÎß?ÎßâÏùå(?¨Ïª§???ÖÎ†•). Î≤ÑÌäº/ÎßÅÌÅ¨ ?Ä?êÏÑú???úÎûòÍ∑∏Î°ú ?§ÌÅ¨Î°?Í∞Ä?? */
  ...
  /**
   * select ?ÑÎìú Ï¶âÏãú ?Ä????savingRef ?ÜÏù¥ ÏßÅÏ†ë DB ?ÖÎç∞?¥Ìä∏ ???¥Îãπ order ?ÑÏ≤¥ Î¶¨ÌîÑ?àÏãú.
   * progress / gift / photo_sent / product_set_type / product_type ???¨Ïö©.
   */
  const quickSaveItem = useCallback(
    async (itemId: string, orderNum: string, field: string, value: unknown) => {
      ...
      const { data, error: fetchErr } = await supabase
        ...
        .single();
      if (fetchErr || !data) {
        showError(fetchErr?.message ?? "?∞Ïù¥???àÎ°úÍ≥†Ïπ® ?§Ìå®");
        setEditing(null);
        return;
      }
  ...
  const runOrderRevert = useCallback(
    async (orderNum: string, payload: Record<string, unknown>) => {
      ...
      const { data, error: fetchErr } = await supabase
        ...
        .single();
      if (fetchErr || !data) {
        showError(fetchErr?.message ?? "Ï£ºÎ¨∏???§Ïãú Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??");
        return;
      }
      setFlatRows((prev) => replaceOrderSegment(prev, orderNum, data as OrderWithNestedItems));
    },
  ...
  const runItemRevertThenRefresh = useCallback(
    async (itemId: string, orderNum: string, updates: Record<string, unknown>) => {
      ...
      if (orderErr || !orderFresh) {
        showError(orderErr?.message ?? "Ï£ºÎ¨∏???§Ïãú Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??");
        return;
      }
    ...
    (
      ...
      if (field === "product_type") {
        if (raw !== "" && !(PRODUCT_CATEGORIES as readonly string[]).includes(raw)) {
          return { error: "Ïπ¥ÌÖåÍ≥†Î¶¨Í∞Ä ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§." };
        }
        return { updates: { product_type: raw === "" ? null : raw } };
      }
      if (field === "product_name") {
        const name = raw.trim();
        if (!name) return { error: "?ÅÌíàÎ™ÖÏ? ÎπÑÏö∏ ???ÜÏäµ?àÎã§." };
      ...
      if (field === "product_set_type") {
        if (!(SET_TYPES as readonly string[]).includes(raw)) return { error: "?®Ìíà/?∏Ìä∏ Í∞íÏù¥ ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§." };
        return { updates: { product_set_type: raw } };
      }
      if (field === "quantity") {
        const q = Math.floor(Number(raw));
        if (!Number.isFinite(q) || q < 1) return { error: "?òÎüâ?Ä 1 ?¥ÏÉÅ?¥Ïñ¥???©Îãà??" };
        return { updates: { quantity: q } };
      }
      if (field === "price_rub") {
        const pr = Number(raw);
        if (!Number.isFinite(pr)) return { error: "?êÎß§Í∞ÄÎ•??ïÏù∏?òÏÑ∏??" };
        return { updates: { price_rub: pr, extra_payment_rub: pr - prep } };
      }
      if (field === "prepayment_rub") {
        const p = Number(raw);
        if (!Number.isFinite(p)) return { error: "?†Í≤∞?úÎ? ?ïÏù∏?òÏÑ∏??" };
        return { updates: { prepayment_rub: p, extra_payment_rub: price - p } };
      }
      if (field === "krw") {
        const t = raw.trim();
        const k = t === "" ? null : Math.round(Number(t));
        if (k !== null && !Number.isFinite(k)) return { error: "?êÌôîÎß§ÏûÖ???ïÏù∏?òÏÑ∏??" };
        return { updates: { krw: k } };
      }
      if (field === "progress") {
        if (!(ORDER_PROGRESS as readonly string[]).includes(raw)) return { error: "ÏßÑÌñâ ?ÅÌÉúÍ∞Ä ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§." };
      ...
      if (field === "photo_sent") {
        if (!(PHOTO_STATUS as readonly string[]).includes(raw)) return { error: "?¨ÏßÑ Î∞úÏÜ° ?ÅÌÉúÍ∞Ä ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§." };
    ...
    <>
      ...
      {historyOpen ? (
        <div className="fixed inset-0 z-[105] flex justify-end bg-black/30" role="presentation">
          <button type="button" className="h-full flex-1 cursor-default" aria-label="?´Í∏∞" onClick={() => setHistoryOpen(false)} />
          <div className="flex h-full w-full max-w-md flex-col border-l border-zinc-200 bg-white shadow-xl dark:border-zinc-700 dark:bg-zinc-950">
            <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
              <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Î≥ÄÍ≤??¥Î†• (ÏµúÍ∑º 10Í∞?</p>
              <button
                type="button"
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {history.length === 0 ? (
                <p className="text-sm text-zinc-500">?ÑÏßÅ Í∏∞Î°ù??Î≥ÄÍ≤ΩÏù¥ ?ÜÏäµ?àÎã§.</p>
              ) : (
                  ...
                  {history.map((e) => (
                    ...
                    >
                      ...
                      <p className="mt-1 text-zinc-800 dark:text-zinc-200">
                        Ï£ºÎ¨∏ {e.orderNum} ¬∑ {e.columnLabel} ¬∑ {e.oldDisplay} ??{e.newDisplay}
                      </p>
                      <button
                        type="button"
                      ...
                      >
                        {undoingId === e.id ? "?òÎèåÎ¶¨Îäî Ï§ë‚Ä? : "?òÎèåÎ¶¨Í∏∞"}
                      </button>
                    </li>
                  ))}
                </ul>
      ...
      >
        Î≥ÄÍ≤??¥Î†• {history.length > 0 ? `(${history.length})` : ""}
      </button>

      {/* ?ÑÌÑ∞ Î∞???crm-subheader-portal (main Î∞îÍπ• sticky ?¨Î°Ø)?ºÎ°ú portal ?åÎçîÎß?*/}
        ...
        <div className="w-full border-b border-zinc-200 bg-white px-4 py-2 dark:border-zinc-800 dark:bg-zinc-950">
          <div className="flex flex-wrap items-center gap-2">
            <FilterDropdown
              label="ÏßÑÌñâ"
              field="progress"
              options={[{ label: "?ÑÏ≤¥", value: "" }, ...ORDER_PROGRESS.map((p) => ({ label: p, value: p }))]}
            />
            <FilterDropdown
              label="?åÎû´??
              field="platform"
              options={[{ label: "?ÑÏ≤¥", value: "" }, ...PLATFORMS.map((p) => ({ label: p, value: p }))]}
            />
            <FilterDropdown
              label="?®Ìíà/?∏Ìä∏"
              field="setType"
            ...
            <FilterDropdown
              label="?¨ÏßÑ"
              field="photoSent"
              options={[{ label: "?ÑÏ≤¥", value: "" }, ...PHOTO_STATUS.map((s) => ({ label: s, value: s }))]}
            />
            <FilterDropdown
              label="?îÍ∏à"
              field="hasBalance"
              options={[
                { label: "?ÑÏ≤¥", value: "" },
                { label: "?îÍ∏à ?àÏùå", value: "yes" },
                { label: "?îÍ∏à ?ÜÏùå", value: "no" },
              ]}
            />
            {hasActiveFilter && (
              <button
            ...
            <input
              type="text"
              placeholder="Ï£ºÎ¨∏Î≤àÌò∏¬∑?ÅÌíàÎ™Ö¬∑Í≥†Í∞ù¬∑Ïòµ??Í≤Ä?â‚Ä?
      ...
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        Ï£ºÎ¨∏ {orderCount}Í±?¬∑ ?úÏãú ??{lineCount}Ï§?¬∑ ?åÏù¥Î∏îÏùÑ ?úÎûòÍ∑∏ÌïòÎ©?Ï¢åÏö∞Î°??§ÌÅ¨Î°§Îê©?àÎã§.
      </p>
        ...
        <table className="min-w-full border-collapse text-left text-sm">
          ...
          <thead>
            <tr>
              ...
              {/* sticky: ?†Ïßú */}
              <th className={`${thClass} sticky top-[108px] z-30`} style={{ left: "32px", width: "46px", minWidth: "46px" }}>?†Ïßú</th>
              {/* sticky: Ï£ºÎ¨∏Î≤àÌò∏ */}
              <th className={`${thClass} sticky top-[108px] z-30`} style={{ left: "78px", width: "90px", minWidth: "90px" }}>Ï£ºÎ¨∏Î≤àÌò∏</th>
              {/* sticky: ?ÅÌíàÎ™?*/}
              <th className={`${thClass} sticky top-[108px] z-30 text-left`} style={{ left: "168px", minWidth: "300px" }}>?ÅÌíàÎ™?/th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[180px] text-left`}>?µÏÖò</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[112px]`}>ÏßÑÌñâ</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[72px]`}>?®Ìíà/?∏Ìä∏</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[52px]`}>?†Î¨º</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[88px]`}>?¨ÏßÑ</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[100px]`}>?ºÏûê</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[72px]`}>?åÎû´??/th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[72px]`}>Í≤ΩÎ°ú</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[100px]`}>Í≥†Í∞ù</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[88px]`}>Í±∞ÎûòÏ≤?/th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[88px]`}>Ïπ¥ÌÖåÍ≥†Î¶¨</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[48px]`}>?òÎüâ</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[88px]`}>?êÎß§Í∞Ä??/th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[88px]`}>?êÌôîÎß§ÏûÖ</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[80px]`}>?†Í≤∞?ú‚ÇΩ</th>
              <th className={`${thClass} sticky top-[108px] z-20 min-w-[72px] border-r-0`}>?îÍ∏à??/th>
            </tr>
          </thead>
          <tbody>
